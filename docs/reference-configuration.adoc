...
| `false`
| Include request params (JSON-RPC `params`) in `nativeCall.requestParams`.
Expensive; use for debugging only.

| `include-response-bodies`
| `false`
| Include response result and error message in `responseBody` / `errorMessage`.
Expensive; use for debugging only.
For errors, only the error message is logged, not the full error response JSON.

| `include-messages`
| `false`
| Deprecated. When `true`, equivalent to setting both `include-request-bodies` and `include-response-bodies` to `true`.
Granular keys override this default when present (e.g. `include-messages: true` with `include-response-bodies: false` logs request params only).
...
| `chains`
| (not set)
| Optional list of blockchains to log (same ids as `proxy.routes` / `upstreams`, e.g. `ethereum`, `bitcoin`).
When omitted, all chains are logged.
When set, only valid listed chains are logged; an empty list or only invalid ids logs nothing (and logs an error at startup).

| `min-latency-ms`
| (not set)
| Optional minimum response time in milliseconds for `NativeCall` lines only.
When omitted or `0`, no latency filter is applied.
When set to a positive value, only `NativeCall` events with `latency` greater than or equal to this threshold are logged (inclusive).
Negative values disable the filter (with a warning at startup).
Does not apply to other access-log event types.
Composes with `chains`.
