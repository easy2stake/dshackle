...
When `chains` is set, only requests for those blockchains are written to the access log.
Omit `chains` to log every chain.
If `chains` is set but empty or contains only unknown ids, nothing is logged and startup reports an error (it does not fall back to logging all chains).

Optional `min-latency-ms` logs only slow `NativeCall` replies: a line is written when `latency` is greater than or equal to the threshold (milliseconds).
Omit it or set `0` to log every `NativeCall` (subject to `chains`).
Negative values are ignored and disable the filter.
Other access-log methods (for example gRPC `Status` or `SubscribeHead`) are not filtered by this option.
For JSON-RPC batch calls, each sub-call is filtered on its own `latency`.

`filename` is optional, and the default value is `access_log.jsonl` (i.e., in the current directory).

Optional `include-request-bodies` and `include-response-bodies` control whether request params and response payloads are written to each line (both default to `false`).
Use them independently when debugging — for example, log request params only, or enable response bodies when investigating slow or failed calls.
Legacy `include-messages: true` enables both; prefer the granular flags for new configs.
...
.Where:
- `ts` timestamp of the reply (for JSON-RPC proxy batch calls, each line has the time that specific sub-call finished)
...
- `latency` response time in milliseconds for this reply: elapsed from `request.start` to `ts` (end-to-end per sub-call, including JSON parsing for HTTP proxy)
- `request` original request details
** `id` uniq id of the request; all replied to the same request have same id
** `start` when the client request was received (HTTP/WS: when the proxy handler was created; gRPC: when the `NativeCall` RPC started)
